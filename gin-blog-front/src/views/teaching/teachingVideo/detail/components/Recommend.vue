...
<template>
  <div v-if="recommendList && recommendList.length" class="space-y-3">
    ...
    <div class="grid grid-cols-12 gap-1.5">
      <div v-for="item of recommendList" :key="item.id" class="group col-span-12 lg:col-span-4">
        <RouterLink :to="`/video/${item.id}`">
          <div class="relative h-[150px] f-c-c overflow-hidden rounded bg-black lg:h-[200px]">
            <img :src="item.cover || convertImgUrl(item.img)" class="h-full w-full object-cover opacity-40 transition-600 group-hover:scale-110 group-hover:opacity-80">
            <div class="absolute text-center text-white">
              ...
              <!-- 播放图标 -->
              <p class="mt-2">
                <span class="i-mdi-play-circle text-3xl" />
              </p>
            </div>
          </div>
        </RouterLink>
